...
export const createClient = (url, token = () => null) => (query, variables = null) => axios(
  createConfig(url, query, variables, token())
).then(response => response.data)


const __client = createClient('http://localhost:3000/api', () => localStorage.getItem('token'))

export const gql = {
  promise: __client,
  observable: (query, variables = null) => Observable.fromPromise(__client(query, variables))
}
